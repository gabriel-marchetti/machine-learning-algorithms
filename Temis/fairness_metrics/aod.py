...
'''
Compute Average Odds Difference (AOD) between privileged and unprivileged groups.
Parameters:
...
Returns:
    float: Average Odds Difference (AOD).
'''
def compute_aod(y_true : np.ndarray, y_pred : np.ndarray, protected_attr : np.ndarray) -> float:
    ...
    aod = 1/2 * ((p_tp_unpr - p_tp_priv) + (p_fp_unpr - p_fp_priv))
